...
sub init {
    ...
    if ( ! -f $self->{opt_extra} ) {
        my $msg = "Can't find config file '$self->{opt_extra}'. If a migration from "
            ...
            . "run perl convert_config_file.pl /etc/centreon/centreon_vmware.pm > /etc/centreon/centreon_vmware.json";
...
sub check_childs {
    ...
    # $self->{logger}->writeLogDebug("Counting children");
    my $count = $self->verify_child_vsphere();
    $self->{logger}->writeLogDebug("$count children found. Stop ? : " . $self->{stop});
    if ($self->{stop} == 1) {
        # No children
        if ($count == 0) {
...
sub run {
    ...
    $centreon_vmware->{logger}->writeLogDebug("Entering loop");
